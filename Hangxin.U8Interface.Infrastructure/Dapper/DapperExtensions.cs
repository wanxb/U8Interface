...
namespace Hangxin.U8Interface.Infrastructure.Dapper
{
    public static class SqlMapperExtensions
    {
        ...
        public static PagedResult<T> GetPageList<T>(this IDbConnection connection, string sql, int pageIndex, int pageSize, object param = null, IDbTransaction transaction = null, int? commandTimeout = null)
        {
            ...
            //SqlServer 分页 SELECT * FROM Table ORDER BY id OFFSET 20 ROWS FETCH NEXT 10 ROWS ONLY
            sql = $"SELECT * FROM ({sql})tt1 ORDER BY rows OFFSET {startRow} ROWS FETCH NEXT {pageSize} ROWS ONLY; SELECT COUNT(1) FROM ({sql}) tt2;";
